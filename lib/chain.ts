...
const chains: Chain[] = []
for await (const dirEntry of await Deno.readDir(`${new URL(import.meta.url).pathname.replace(/[^\/]*$/, 'ethereum-lists')}/chains/_data/chains`)) {
    const uint8Array = await Deno.readFile(`${new URL(import.meta.url).pathname.replace(/[^\/]*$/, 'ethereum-lists')}/chains/_data/chains/${dirEntry.name}`)
